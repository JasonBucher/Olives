...
describe("computeRunAnalysis", () => {
  ...
  it("extracts points from state_snapshot events", () => {
    const events = normalizeEvents([
      { ms: 1000, type: "state_snapshot", payload: { avocadoCount: 10, aps: 1, guacCount: 0, wisdom: 0, prestigeCount: 0, reason: "tick" } },
      { ms: 2000, type: "purchase", payload: { kind: "producer", id: "sapling", title: "Sapling" } },
      { ms: 3000, type: "state_snapshot", payload: { avocadoCount: 50, aps: 5, guacCount: 2, wisdom: 0, prestigeCount: 0, reason: "tick" } },
    ]);
    const r = computeRunAnalysis(events);
    expect(r.points).toHaveLength(2);
    expect(r.points[0].avocadoCount).toBe(10);
    expect(r.points[1].avocadoCount).toBe(50);
    expect(r.points[1].guacCount).toBe(2);
  });

  it("computes metadata from first and last events", () => {
    const events = normalizeEvents([
      { ms: 1000, type: "state_snapshot", payload: { avocadoCount: 0 }, sessionId: "sess-123" },
      { ms: 6000, type: "state_snapshot", payload: { avocadoCount: 100 }, sessionId: "sess-123" },
    ]);
    const r = computeRunAnalysis(events);
  ...
  it("builds timeline rows and marks key events", () => {
    ...
    expect(r.timelineRows[0].isKey).toBe(true); // state_snapshot
    expect(r.timelineRows[1].isKey).toBe(true); // purchase
    expect(r.timelineRows[2].isKey).toBe(true); // prestige
    expect(r.timelineRows[2].details).toContain("5");
  });

  it("computes summary from last snapshot", () => {
    const events = normalizeEvents([
      { ms: 1000, type: "state_snapshot", payload: { avocadoCount: 10, guacCount: 0, wisdom: 0, prestigeCount: 0 } },
      { ms: 5000, type: "state_snapshot", payload: { avocadoCount: 999, guacCount: 42, wisdom: 7, prestigeCount: 3 } },
    ]);
    const r = computeRunAnalysis(events);
    expect(r.summary.finalAvocados).toBe(999);
    expect(r.summary.finalGuac).toBe(42);
    expect(r.summary.finalWisdom).toBe(7);
    expect(r.summary.prestigeCount).toBe(3);
  ...
  it("computes tRelSec relative to first event", () => {
    const events = normalizeEvents([
      { ms: 10000, type: "state_snapshot", payload: { avocadoCount: 0 } },
      { ms: 15000, type: "state_snapshot", payload: { avocadoCount: 100 } },
    ]);
    const r = computeRunAnalysis(events);
...
describe("end-to-end analysis pipeline", () => {
  it("processes NDJSON through the full pipeline", () => {
    const lines = [
      ...
      JSON.stringify({ t: "2026-01-15T10:00:01Z", ms: 2000, sessionId: "s1", type: "state_snapshot", payload: { avocadoCount: 0, aps: 0, guacCount: 0, wisdom: 0, prestigeCount: 0, reason: "init" } }),
      JSON.stringify({ t: "2026-01-15T10:00:05Z", ms: 6000, sessionId: "s1", type: "purchase", payload: { kind: "producer", id: "sapling", title: "Sapling", count: 1, cost: 10 } }),
      JSON.stringify({ t: "2026-01-15T10:00:06Z", ms: 7000, sessionId: "s1", type: "state_snapshot", payload: { avocadoCount: 50, aps: 0.2, guacCount: 0, wisdom: 0, prestigeCount: 0, reason: "tick" } }),
    ];
    ...
    expect(analysis.points[1].avocadoCount).toBe(50);
    expect(analysis.points[1].aps).toBe(0.2);
    expect(analysis.summary.finalAvocados).toBe(50);
