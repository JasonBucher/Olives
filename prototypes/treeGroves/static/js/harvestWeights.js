 ...
 * @param {Object} params.upgrades - Upgrade flags (improved_harvesting, etc.)
...
function computeHarvestOutcomeWeights({ outcomes, harvesterCount, arboristIsActive, upgrades, tuning }) {
  ...
  const efficientImprovedHarvestingBonus = tuning.upgrades?.improved_harvesting?.efficientBonus ?? 0.06;
  const efficientImprovedHarvestingPerHarvester = tuning.upgrades?.improved_harvesting?.efficientPerHarvester ?? 0.01;
  const efficientImprovedHarvestingCap = tuning.upgrades?.improved_harvesting?.efficientCap ?? 0.08;
  ...
  // improved_harvesting adds flat bonus + bonus scaling with harvesters (capped)
  if (upgrades.improved_harvesting) {
    efficientBonus += efficientImprovedHarvestingBonus;
    const scaledBonus = Math.min(harvesterCount * efficientImprovedHarvestingPerHarvester, efficientImprovedHarvestingCap);
